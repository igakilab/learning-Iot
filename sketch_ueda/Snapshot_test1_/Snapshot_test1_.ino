...
void setup() {
  ...
  while (!Console);
  SD.begin();
  Console.println("VC0706 Camera snapshot test");
  ...
  if (cam.begin()) {
    Console.println("Camera Found:");
  } else {
    Console.println("No camera found?");
    return;
  }
  // Print out the camera version information (optional)
  char *reply = cam.getVersion();
  if (reply == 0) {
    Console.print("Failed to get version");
  } else {
    Console.println("-----------------");
    Console.print(reply);
    Console.println("-----------------");
  ...
  Console.print("Image size: ");
  if (imgsize == VC0706_640x480) Console.println("640x480");
  if (imgsize == VC0706_320x240) Console.println("320x240");
  if (imgsize == VC0706_160x120) Console.println("160x120");

  Console.println("Snap in 3 secs...");
  delay(3000);

  if (! cam.takePicture()) 
    Console.println("Failed to snap!");
  else 
    Console.println("Picture taken!");
  ...
  Console.print("Storing ");
  Console.print(jpglen, DEC);
  Console.print(" byte image.");
  ...
  Console.print(time); Console.println(" ms elapsed");
